...
class ExMain(QWidget):
    def __init__(self):
        ...
        btnRun3.clicked.connect(self.creat)
        btnRun3.move(220, 0)
    ...
    def delete(self):
        ...
        print("ddd",test3_list)

        for i, data in enumerate(test4_list):
            print('index',i,"data",data)
            last_pos=test3_list[0]
            print(data,last_pos,data==last_pos)
            if(data==last_pos):
                print("test4_list",test4_list)
                del self.mymousePoints[len(self.mymousePoints)-1]
                self.mpt.setData(pos=self.mymousePoints)
