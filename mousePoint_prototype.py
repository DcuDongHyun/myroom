...
class ExMain(QWidget):
    ...
    def onClick(self, event):
        ...
        if(key_event == True and event.button() == 1):
            ...
            self.linedrawpoint.append(self.draw)
            print(self.linedrawpoint)
            print(self.draw.size)
            if self.draw.size % 4 == 0:
                self.graph_item.setData(pos=self.draw, adj=self.adj)
                self.adjcount += 2
                self.adj = np.append(self.adj, np.array([[self.adjcount, self.adjcount + 1]]), axis=0)
