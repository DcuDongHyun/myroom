...
class ExMain(QWidget):
    def __init__(self):
        ...
        self.btnRun4 = QPushButton("직선 되돌리기", self)
        self.btnRun4.clicked.connect(self.linedel)
    ...
    def linedel(self):
        print("드로우 좌표",self.draw, "드로우 사이즈",self.draw.size)
        print("adj좌표",self.adj,"adj 크기",self.adj.size)
        print("adjcount ",self.adjcount)
        drawsize = self.draw.size
        self.draw = np.delete(self.draw, drawsize/2 -1, 0)
        print("삭제 후 draw 값", self.draw)
        adjsize = self.adj.size
        self.adj = np.delete(self.adj, adjsize/2-1, 0)
        self.adjcount -= 1
        print("삭제 후 adj값 ",self.adj, "감소한 adjcount 값",self.adjcount)
        self.graph_item.setData(pos= self.draw, adj=self.adj)
