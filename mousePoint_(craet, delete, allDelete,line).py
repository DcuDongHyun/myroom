...
class ExMain(QWidget):
    def __init__(self):
        ...
        self.draw_pos = list()
        self.keycount = 1
        # self.linedrawpoint = np.array([[]],dtype='float64')
        self.linedrawpoint = np.empty((0, 2),dtype='float64')
        ...
        self.btnRun4 = QPushButton("직선 그리기", self)
        self.btnRun4.clicked.connect(self.draw_line)
    ...
    def onClick(self, event):
        ...
        if(key_event == True and event.button() == 1):
            ...
            print("point ",point)
            self.linedrawpoint = np.append(self.linedrawpoint, point,axis=0)
            print("컨트롤 누른후",self.linedrawpoint)
            if self.linedrawpoint.size == 4:
                print("조건문 실행 ", self.linedrawpoint)
                self.graph_item.setData(pos=self.linedrawpoint, adj=adj)
        elif(event.button() == 1):
            self.event = event
    ...
    def draw_line(self):
        event = self.event
        mouse_control = self.view.mapSceneToView(event._scenePos)
        pt_draw_pos = [mouse_control.x(), mouse_control.y()]
        self.draw_pos.append(pt_draw_pos)
        it_draw_pos = self.draw_pos
        it_mouse_point = self.mymousePoints
        chang_draw_pos = [[int(i[0]), int(i[1])] for i in it_draw_pos]
        chang_mouse_point = [[int(i[0]), int(i[1])] for i in it_mouse_point]
        print("chang_draw",chang_draw_pos)
        print("chang_mouse",chang_mouse_point)
        print("*"*100)
        for i, data in enumerate(chang_mouse_point):
            if(data == chang_draw_pos):
                print("*"*50)
                print(chang_draw_pos)
